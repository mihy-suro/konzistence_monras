...
data <- read_excel("./Polozky_PR/Lesní Plody ostatní 2023.xlsx", sheet=2, guess_max=21474836)
...
statscs<-paste("Lesní plody ostatní: Cs-137 D-statistika = ",signif(as.numeric(kscs[1]),4),";  p-hodnota = ",signif(as.numeric(kscs[2]),4)) 
...
###################################
pngfile = paste("lesni_plody_ostatni_cs137.png",sep="")
...
limits <- paste("Lesní plody ostatní:  Prùmìr = ", signif(mean(newoldcs$a),3),";  TI90 = ",signif(ti90[5],3),";  TI95 = ",signif(ti95[5],3), ";  TI99 = ",signif(ti99[5],3),sep="")
...
leftalign<-newoldcs$od[length(newoldcs$od)-50]
for(i in 1:length(outliers)){
  text(newoldcs$od[outliers[i]],newoldcs$a[outliers[i]]-0.3,outind[i])  
  text(leftalign,6-0.5*i,outlabels[i], pos = 4)  
}
