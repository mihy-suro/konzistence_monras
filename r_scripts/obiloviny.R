...
data <- read_excel("./Polozky_PR/Obiloviny 2023.xlsx", sheet=2, guess_max=21474836)
...
statscs<-paste("Obiloviny: Cs-137 D-statistika = ",signif(as.numeric(kscs[1]),4),";  p-hodnota = ",signif(as.numeric(kscs[2]),4)) 
...
###################################
pngfile = paste("obiloviny_cs137.png",sep="")
...
limits <- paste("Obiloviny:  Prùmìr = ", signif(mean(newoldcs$a),3),";  TI90 = ",signif(ti90[5],3),";  TI95 = ",signif(ti95[5],3), ";  TI99 = ",signif(ti99[5],3),sep="")
...
plot(newoldcs$od,newoldcs$a, col= colors[1], pch = 19, cex=2, xlab="Datum", ylab="Aktivita [Bq/kg]",
     cex.lab=2, cex.axis=2, ylim=c(0,yhigh))
points(newoldmvacs$od,newoldmvacs$a, col= "green", pch = 19, cex=1.5)
...
leftalign<-newoldcs$od[1]
for(i in 1:length(outliers)){
  text(newoldcs$od[outliers[i]],newoldcs$a[outliers[i]]+0.1,outind[i])  
  text(leftalign,1-0.05*i,outlabels[i], pos = 4)  
}
...
###################################
pngfile = paste("obiloviny_sr90.png",sep="")
png(filename=pngfile,width = 1100, height = 1000, units = "px")
...
limits <- paste("Obiloviny:  Prùmìr = ", signif(mean(newoldsr$a),3),";  TI90 = ",signif(ti90[5],3),";  TI95 = ",signif(ti95[5],3), ";  TI99 = ",signif(ti99[5],3),sep="")
...
plot(newoldsr$od,newoldsr$a, col= colors[1], pch = 19, cex=2, xlab="Datum", ylab="Aktivita [Bq/kg]",
     ylim=c(0,yhigh), cex.axis=2, cex.lab=2)
