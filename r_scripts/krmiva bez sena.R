...
data <- read_excel("./Polozky_PR/Krmiva bez sena 2023.xlsx", sheet=2, guess_max=21474836)

kom_ID<-data$ID_Monit_položka_OM 
kom<-trimws(sapply(strsplit(data$Monitorovaná_položka,"/"), tail, 1))
...
statscs<-paste("Krmiva: Cs-137 D-statistika = ",signif(as.numeric(kscs[1]),4),";  p-hodnota = ",signif(as.numeric(kscs[2]),4)) 
...
###################################
pngfile = paste("krmiva_bezsena_cs137.png",sep="")
...
limits <- paste("Krmiva:  Prùmìr = ", signif(mean(newoldcs$a),3),";  TI90 = ",signif(ti90[5],3),";  TI95 = ",signif(ti95[5],3), ";  TI99 = ",signif(ti99[5],3),sep="")
outliers<-which(newoldcs$a>1)
...
plot(newoldcs$od,newoldcs$a, col= colors[1], pch = 19, cex=2, cex.axis = 2, cex.lab=2, xlab="Datum",
     ylab="Aktivita [Bq/kg]", ylim=c(0,yhigh))
...
leftalign<-newoldcs$od[length(newoldcs$od)-200]
for(i in 1:length(outliers)){
  text(newoldcs$od[outliers[i]],newoldcs$a[outliers[i]]+0.3,outind[i])  
  text(leftalign,7.5-0.5*i,outlabels[i], pos = 3)  
}
