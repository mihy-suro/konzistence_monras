...
data <- read_excel("./Polozky_PR/Okopaniny 2023.xlsx", sheet=2, guess_max=21474836)
...
statscs<-paste("Okopaniny: Cs-137 D-statistika = ",signif(as.numeric(kscs[1]),4),";  p-hodnota = ",signif(as.numeric(kscs[2]),4)) 
...
###################################
pngfile = paste("okopaniny_cs137.png",sep="")
...
limits <- paste("Okopaniny:  Prùmìr = ", signif(mean(newoldcs$a),3),";  TI90 = ",signif(ti90[5],3),";  TI95 = ",signif(ti95[5],3), ";  TI99 = ",signif(ti99[5],3),sep="")
...
for(i in 1:length(outliers)){
  text(newoldcs$od[outliers[i]],newoldcs$a[outliers[i]]+0.025,outind[i])  
  text(leftalign,0.6-0.06*i,outlabels[i], pos = 4)  
}
