...
data <- read_excel("./Polozky_PR/Ryby 2023.xlsx", sheet=2, guess_max=21474836)
...
statscs<-paste("Ryby: Cs-137 D-statistika = ",signif(as.numeric(kscs[1]),4),";  p-hodnota = ",signif(as.numeric(kscs[2]),4)) 
...
###################################
pngfile = paste("ryby_cs137.png",sep="")
...
limits <- paste("Ryby:  Prùmìr = ", signif(mean(newoldcs$a),3),";  TI90 = ",signif(ti90[5],3),";  TI95 = ",signif(ti95[5],3), ";  TI99 = ",signif(ti99[5],3),sep="")
outliers<-which(newoldcs$a>2.4)

outind<-c(1:length(outliers))
outlabels<-paste(outind, " ... " ,newoldcs$kom[outliers],"/",newoldcs$om[outliers])
yhigh<-max(max(newoldcs$a)*1.1,as.numeric(ti99[5])*1.1)
...
for(i in 1:length(outliers)){
  ...
  text(leftalign,7-0.3*i,outlabels[i], pos = 4)  
}
