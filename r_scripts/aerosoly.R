...
data <- read_excel("./Polozky_ZP/Aerosoly 2023.xlsx", sheet=1, guess_max=21474836)
...
end = "2023-12-31 23:59:59"             # èasový výbìr do
...
all<-all[all$om %in% c("Brno", "Brno - Arboretum","Èeské Budìjovice - U nemocnice","Holešov - letištì",
                  ...
                  "Plzeò - Klatovská", "Praha - Bartoškova","Ústí nad Labem - Habrovice"), ] 
...
#==============================================================================
# GRAFY Dolní Rožínka - pouze rok 2022 - 2023
#==============================================================================
...
dr<-ff(tmpdf,kom, om, lab, rn, start, end)
dr$year<-year(dr$od)

year <- dr$year
drmva<-dr[which(dr$mva==1),]
yearmva <- as.numeric(format(drmva$od,'%Y'))

pngfile = paste("aerosoly_dolni_rozinka_boxplot.png",sep="")
png(filename=pngfile,width = 1100, height = 1100, units = "px")

par(mfrow=c(2,1), mai = c(1, 1.25, 0.25, 0.1))

colnew="red"
jittercol <- rgb(0, 0, 255, max = 255, alpha = 50)
jittercolmva <- rgb(0, 255, 0, max = 255, alpha = 50)

boxplot(dr$a ~ dr$year, outline = FALSE,ylab="Aktivita[Bq/m3]",xlab="Datum",
        boxlwd = 2,whisklwd=2,staplelwd=2, cex.lab = 2, cex.axis=2, cex=2)
# stripchart(dr$a ~ dr$year, vertical = TRUE, data = dr, 
#            method = "jitter", add = TRUE, pch = 20, col = jittercol, cex=2)
# stripchart(drmva$a ~ yearmva, vertical = TRUE, data = dr, 
#            method = "jitter", add = TRUE, pch = 20, col = jittercolmva, cex=2)


ti90<-normtol.int(dr$a, alpha = 0.05, P = 0.90, side = 1, log.norm = TRUE)
ti95<-normtol.int(dr$a, alpha = 0.05, P = 0.95, side = 1, log.norm = TRUE)
ti99<-normtol.int(dr$a, alpha = 0.05, P = 0.99, side = 1, log.norm = TRUE)
limits <- paste("Prùmìr = ", signif(mean(dr$a),3),";  TI90 = ",signif(ti90[5],3),";  TI95 = ",signif(ti95[5],3), ";  TI99 = ",signif(ti99[5],3),sep="")

colors = c(rep("grey",length(unique(year))))
font_size = 2
point_size = 2

plot(dr$od,dr$a, col= colors[1], pch = 19,  cex = 2, xlab="Datum", 
     ylab="Aktivita [Bq/m3]",cex.lab = font_size, cex.axis=font_size, xaxt = "n")
axis.POSIXct(1, at=seq(dr$od[1], dr$od[length(dr$od)], by="month"), format="%Y/%m")
points(dr$od,dr$a, col = colors[1], pch=19,  cex = 2)
points(drmva$od,drmva$a, col = jittercolmva, pch=19, cex = 2)
...
stats<-paste("Aerosoly - všechna data,   D-statistika = ",signif(as.numeric(ks[1]),4),";  p-hodnota = ",signif(as.numeric(ks[2]),4))
colnew="red"
jittercol <- rgb(0, 0, 255, max = 255, alpha = 50)
jittercolmva <- rgb(0, 255, 0, max = 255, alpha = 50)
...
pngfile = paste("aerosoly_boxplot_vse_",rnfile,".png",sep="")
png(filename=pngfile,width = 1100, height = 800, units = "px")

par(mai = c(1, 1.25, 1, 0.1))
colors = c(rep("grey",length(unique(newold$year))-1),colnew)
boxplot(newold$a ~ newold$year, outline = FALSE,ylab="Aktivita[Bq/m3]",xlab="Datum", col=colors,
        boxlwd = 2,whisklwd=2,staplelwd=2, cex.lab = 2, cex.axis=2, cex=2)
# stripchart(newold$a ~ newold$year, vertical = TRUE, data = newold, 
...
pngfile = paste("aerosoly_boxplot_",oms[i],"_",rnfile,".png",sep="")
...
plot(newold$od[labind],newold$a[labind], col= colors[1], cex = 2, pch = 19, xlab="Datum", 
     ylab="Aktivita[Bq/m3]", ylim = c(0,yzoom), cex.lab = font_size, cex.axis=font_size)
...
pngfile = paste("aerosoly_vse_qqplot_",rnfile,".png",sep="")
png(filename=pngfile,width = 1100, height = 800, units = "px")
plot(qq$x,qq$y, xlab="kvantily normálního rozdìlení", ylab="ln(Aktivita[Bq/m3])", pch=1,
...
legend("topleft", legend=c("Referenèní data", "Aktuální data"), col=c("black", "red"), 
       pch=c(1,0),cex=2)
meta<-paste("Aerosoly  všechna data   ",rn, "   Data od:", as.Date(newdata), "    Ref. data od: ", as.Date(olddata),sep=" ")
...
for (i in 1:nlab){
  ...
  pngfile = paste("aerosoly_qqplot",oms[i],"_",rnfile,".png",sep="")
  png(filename=pngfile,width = 1100, height = 800, units = "px")
  ...
  meta<-paste("Aerosoly  ",oms[i]," ",rn, "   Data od:", as.Date(newdata), "    Ref. data od: ", as.Date(olddata),sep=" ")
  mtext(meta, side=3, line=0, cex=2)
  dev.off()
}
...
# dìlicí body pro referenèní a aktuální data:
newdata = "2022-12-26 12:00:00 CET"
olddata = "2013-01-01 00:00:00 CET"
...
all$year <- year(all$do)
# !!!!!!!!!!! Pozor, tady jsem dal "do", jinak by vzorek z pøelomu roku spadnul do pøedchozího...
...
pngfile = paste("aerosoly_boxplot","_",rnfile,".png",sep="")
png(filename=pngfile,width = 1100, height = 1100, units = "px")

par(mfrow=c(2,1), mai = c(1, 1.25, 1, 0.1))
...
#            method = "jitter", add = TRUE, pch = 20, col = jittercolmva, cex=2)
mtext(stats, side=3, line=0, cex=2)
mtext("Sr-90", side=3, line=2, cex=1.5)
...
plot(newold$od,newold$a, col= "grey", pch = 19,  cex = 2, xlab="Datum", 
     ylab="Aktivita[Bq/m3]",cex.lab = font_size, cex.axis=font_size, ylim=c(0,yzoom))
...
#            method = "jitter", add = TRUE, pch = 20, col = jittercolmva, cex=2)
mtext(stats, side=3, line=0, cex=2)
mtext("Pu-238", side=3, line=2, cex=1.5)
...
mtext("Pu-239+240", side=3, line=2, cex=1.5)
