...
data <- read_excel("./Polozky_PR/Borùvky brusinky 2023.xlsx", sheet=2, guess_max=21474836)
...
statscs<-paste("Borùvky, brusinky: Cs-137 D-statistika = ",signif(as.numeric(kscs[1]),4),";  p-hodnota = ",signif(as.numeric(kscs[2]),4)) 
...
###################################
pngfile = paste("boruvkybrusinky_cs137.png",sep="")
...
limits <- paste("Borùvky, brusinky:  Prùmìr = ", signif(mean(newoldcs$a),3),";  TI90 = ",signif(ti90[5],3),";  TI95 = ",signif(ti95[5],3), ";  TI99 = ",signif(ti99[5],3),sep="")
