...
data <- read_excel("./Polozky_PR/Potravináøské výrobky bez medu 2023.xlsx", sheet=2, guess_max=21474836)
...
###################################
pngfile = paste("vyrobky_bezmedu_cs137.png",sep="")
...
leftalign<-newoldcs$od[length(newoldcs$od)-300]
for(i in 1:length(outliers)){
text(newoldcs$od[outliers[i]],newoldcs$a[outliers[i]]+0.05,outind[i])  
text(leftalign,1.1-0.05*i,outlabels[i], pos = 3)  
}
