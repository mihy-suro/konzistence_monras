...
data <- read_excel("./Polozky_PR/Potravináøské výrobky 2023.xlsx", sheet=2, guess_max=21474836)
...
statscs<-paste("Potrav. výrobky: Cs-137 D-statistika = ",signif(as.numeric(kscs[1]),4),";  p-hodnota = ",signif(as.numeric(kscs[2]),4)) 
...
###################################
pngfile = paste("vyrobky_cs137.png",sep="")
...
limits <- paste("Potrav. výrobky:  Prùmìr = ", signif(mean(newoldcs$a),3),";  TI90 = ",signif(ti90[5],3),";  TI95 = ",signif(ti95[5],3), ";  TI99 = ",signif(ti99[5],3),sep="")
...
plot(newoldcs$od,newoldcs$a, pch = 19, cex=2, xlab="Datum", ylab="Aktivita [Bq/kg]", col = jittercol, 
     ylim=c(0,max(newoldcs$a)*1.1), cex.lab = 2, cex.axis=2)
...
for(i in 1:length(outliers)){
  text(newoldcs$od[outliers[i]],newoldcs$a[outliers[i]]+0.5,outind[i])  
...
yearmvacs <- as.numeric(format(newoldmvacs$od,'%Y'))
kscs <- ks.test(oldcs$a,newcs$a)
statscs<-paste("Med: Cs-137 D-statistika = ",signif(as.numeric(kscs[1]),4),";  p-hodnota = ",signif(as.numeric(kscs[2]),4)) 
...
###################################
pngfile = paste("med_cs137.png",sep="")
png(filename=pngfile,width = 1100, height = 1000, units = "px")
...
ti95<-normtol.int(newoldcs$a, alpha = 0.05, P = 0.95, side = 1, log.norm = TRUE)
ti99<-normtol.int(newoldcs$a, alpha = 0.05, P = 0.99, side = 1, log.norm = TRUE)
limits <- paste("Med:  Prùmìr = ", signif(mean(newoldcs$a),3),";  TI90 = ",signif(ti90[5],3),";  TI95 = ",signif(ti95[5],3), ";  TI99 = ",signif(ti99[5],3),sep="")
